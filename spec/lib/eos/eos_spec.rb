describe Eos::Blockchain do
    context :features do
      it 'defaults' do
        blockchain1 = Eos::Blockchain.new
        expect(blockchain1.features).to eq Eos::Blockchain::DEFAULT_FEATURES
      end
  
      it 'override defaults' do
        blockchain2 = Eos::Blockchain.new(supports_cash_addr_format: true)
        expect(blockchain2.features[:supports_cash_addr_format]).to be_truthy
      end
  
      it 'custom feautures' do
        blockchain3 = Eos::Blockchain.new(custom_feature: :custom)
        expect(blockchain3.features.keys).to contain_exactly(:supports_cash_addr_format, :case_sensitive)
      end
    end
  
    context :configure do
      let(:blockchain) { Eos::Blockchain.new }
      ...
      it 'currencies and server configuration' do
        ...
        settings = { server: 'http://127.0.0.1:8888',
    ...
    context :latest_block_number do
      ...
      let(:blockchain) do
        Eos::Blockchain.new.tap {|b| b.configure(server: server)}
      end
  
      it 'returns latest block number' do
        block_number = 1489174
  
        stub_request(:post, 'http://127.0.0.1:8888')
      ...
      it 'raises error if there is error in response body' do
        ...
        expect{ blockchain.latest_block_number }.to raise_error(Eos::Client::ResponseError)
      end
    end
    ...
    context :fetch_block! do
      ...
      let(:block_data) do
        Rails.root.join('spec', 'resources', 'eos-data', block_file_name)
      ...
      let(:server) { 'http://user:password@127.0.0.1:8888' }
      let(:endpoint) { 'http://127.0.0.1:8888' }
      let(:blockchain) do
        Eos::Blockchain.new.tap { |b| b.configure(server: server, currencies: [currency]) }
      end
    ...
    context :build_transaction do
      ...
      let(:tx_hash) do
        Rails.root.join('spec', 'resources', 'eos-data', tx_file_name)
          .yield_self { |file_path| File.open(file_path) }
          .yield_self { |file| JSON.load(file) }
      end
      ...
      let(:blockchain) do
        Eos::Blockchain.new.tap { |b| b.configure(currencies: [currency.to_blockchain_api_settings]) }
      end
