describe Worker::Matching do
  ...
  context 'partial match' do
    ...
    it 'should started engine' do
      expect(subject.engines["btc#{Peatio.base_fiat_ccy.downcase}"].mode).to eq :run
    end
    ...
    it 'should match part of existing order' do
      ...
      AMQPQueue.expects(:enqueue)
               .with(:slave_book, { action: 'update', order: { id: existing.id, timestamp: existing.at, type: :ask, volume: '2.0'.to_d, price: existing.price, market: "btc#{Peatio.base_fiat_ccy.downcase}", ord_type: 'limit' } }, anything)
      AMQPQueue.expects(:enqueue)
  ...
  context 'dryrun' do
    ...
    context 'very old orders matched' do
      ...
      it 'should not start engine' do
        expect(subject.engines["btc#{Peatio.base_fiat_ccy.downcase}"].mode).to eq :dryrun
        expect(subject.engines["btc#{Peatio.base_fiat_ccy.downcase}"].queue.size).to eq 1
      end
    end
