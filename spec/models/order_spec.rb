describe Order, 'validations', type: :model do
  ...
  context 'limit order' do
    it 'should make sure price is present' do
      order = Order.new(currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: nil, ord_type: 'limit')
      expect(order).not_to be_valid
      expect(order.errors[:price]).to eq ['is not a number']
    end

    it 'should make sure price is greater than zero' do
      order = Order.new(currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '0.0'.to_d, ord_type: 'limit')
      expect(order).not_to be_valid
  ...
  context 'market order' do
    it 'should make sure price is not present' do
      order = Order.new(currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '0.0'.to_d, ord_type: 'market')
      expect(order).not_to be_valid
...
describe Order, '#fix_number_precision', type: :model do
  let(:order_bid) { create(:order_bid, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '12.326'.to_d, volume: '123.123456789') }
  let(:order_ask) { create(:order_ask, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '12.326'.to_d, volume: '123.123456789') }
...
describe Order, 'related accounts' do
  ...
  context OrderAsk do
    it "should hold btc and expect #{Peatio.base_fiat_ccy.downcase}" do
      ask = create(:order_ask, member: alice)
      expect(ask.hold_account).to eq alice.get_account(:btc)
      expect(ask.expect_account).to eq alice.get_account(Peatio.base_fiat_ccy_sym.downcase)
    end
  end

  context OrderBid do
    it "should hold #{Peatio.base_fiat_ccy.downcase} and expect btc" do
      bid = create(:order_bid, member: bob)
      expect(bid.hold_account).to eq bob.get_account(Peatio.base_fiat_ccy_sym.downcase)
