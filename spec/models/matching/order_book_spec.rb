describe Matching::OrderBook do
  context '#find' do
    subject { Matching::OrderBook.new("btc#{Peatio.base_fiat_ccy.downcase}", :ask) }
  ...
  context '#add' do
    ...
    it 'should broadcast add event' do
      ...
      AMQPQueue.expects(:enqueue).with(:slave_book, { action: 'new', market: "btc#{Peatio.base_fiat_ccy.downcase}", side: :ask }, persistent: false)
    ...
    it 'should not broadcast add event' do
      ...
      Matching::OrderBook.new("btc#{Peatio.base_fiat_ccy.downcase}", :ask, broadcast: false).add order
    end
  end
  ...
  context '#best_limit_price' do
    it 'should return highest bid price' do
      book = Matching::OrderBook.new("btc#{Peatio.base_fiat_ccy.downcase}", :bid)
    ...
    it 'should return lowest ask price' do
      book = Matching::OrderBook.new("btc#{Peatio.base_fiat_ccy.downcase}", :ask)
      o1   = Matching.mock_limit_order(type: :ask, price: '1.0'.to_d)
