...
describe Private::OrderAsksController, type: :controller do
  ...
  context 'POST :create' do
    it 'should create a sell order' do
      expect do
        post :create, params, member_id: account_btc.member.id
  ...
  context 'POST :clear' do
    it 'should cancel all my asks in current market' do
      o1 = create(:order_ask, member: account_btc.member, market: market)
      o2 = create(:order_ask, member: account_btc.member, market: Market.find(:dashbtc))
      expect(member.orders.size).to eq 2

      post :clear, { market_id: market.id }, member_id: account_btc.member.id
      expect(response).to be_success
