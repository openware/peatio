    ...
    context 'deposit' do
      context 'deposit wallet doesnt exist' do
        it do
          ...
          expect(Wallet.where(status: %w[active retired], kind: :deposit, gateway: 'opendax_cloud').count).to eq 0
        end
      end
          ...
          context 'accepted deposits exist' do
            ...
            context 'there is payment address' do
              before do
                ...
                Wallet.deposit_wallet(transaction.currency_id).update(status: 'disabled') if Wallet.deposit_wallet(transaction.currency_id).gateway == 'geth'
    ...
    context 'withdraw' do
      context 'hot wallet doesnt exist' do
        it do
          ...
          expect(Wallet.where(status: %w[active retired], kind: :hot, gateway: 'opendax_cloud').count).to eq 0
        end
      end
    ...
    context 'deposit address' do
      ...
      context 'deposit wallet exists' do
        ...
        context 'event exists' do
          ...
          before do
            # disable first deposit wallet for eth to have ability to use opendax cloud deposit wallet
            Wallet.deposit_wallet('eth').update(status: 'disabled') if Wallet.deposit_wallet('eth').gateway == 'geth'
