describe APIv2::Orders, type: :request do
  ...
  describe 'GET /api/v2/orders' do
    before do
      create(:order_bid, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '11'.to_d, volume: '123.123456789', member: member)
      create(:order_bid, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '12'.to_d, volume: '123.123456789', member: member, state: Order::CANCEL)
      create(:order_ask, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '13'.to_d, volume: '123.123456789', member: member)
      create(:order_ask, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '14'.to_d, volume: '123.123456789', member: member, state: Order::DONE)
    end

    it 'should require authentication' do
      get '/api/v2/orders', market: "btc#{Peatio.base_fiat_ccy.downcase}"
    ...
    it 'should validate state param' do
      signed_get '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", state: 'test' }, token: token
    ...
    it 'should return active orders by default' do
      signed_get '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}" }, token: token
    ...
    it 'should return complete orders' do
      signed_get '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", state: Order::DONE }, token: token
    ...
    it 'should return paginated orders' do
      signed_get '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", limit: 1, page: 1 }, token: token

      expect(response).to be_success
      expect(JSON.parse(response.body).first['price']).to eq '11.0'

      signed_get '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", limit: 1, page: 2 }, token: token
    ...
    it 'should sort orders' do
      signed_get '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", order_by: 'asc' }, token: token
      expect(response).to be_success
      orders = JSON.parse(response.body)
      expect(orders[0]['id']).to be < orders[1]['id']

      signed_get '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", order_by: 'desc' }, token: token
      expect(response).to be_success
      orders = JSON.parse(response.body)
      expect(orders[0]['id']).to be > orders[1]['id']
    end
  ...
  describe 'GET /api/v2/order' do
    let(:order)  { create(:order_bid, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '12.326'.to_d, volume: '3.14', origin_volume: '12.13', member: member, trades_count: 1) }
  ...
  describe 'POST /api/v2/orders/multi' do
    before do
      ...
      member.get_account(Peatio.base_fiat_ccy_sym.downcase).update_attributes(balance: 100_000)
    end
  ...
  describe 'POST /api/v2/orders' do
    it 'should create a sell order' do
      ...
      expect do
        signed_post '/api/v2/orders', token: token, params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", side: 'sell', volume: '12.13', price: '2014' }
        expect(response).to be_success
    ...
    it 'should create a buy order' do
      ...
      expect do
        signed_post '/api/v2/orders', token: token, params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", side: 'buy', volume: '12.13', price: '2014' }
        expect(response).to be_success
    ...
    it 'should return cannot lock funds error' do
      ...
      signed_post '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", side: 'sell', volume: '12.13', price: '2014' }
      expect(response.code).to eq '400'
    ...
    it 'should give a number as volume parameter' do
      signed_post '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", side: 'sell', volume: 'test', price: '2014' }
      expect(response.code).to eq '400'
    ...
    it 'should give a number as price parameter' do
      signed_post '/api/v2/orders', params: { market: "btc#{Peatio.base_fiat_ccy.downcase}", side: 'sell', volume: '12.13', price: 'test' }
      expect(response.code).to eq '400'
      expect(response.body).to eq '{"error":{"code":2002,"message":"Failed to create order. Reason: Validation failed: Price must be greater than 0"}}'
    end
  end
  ...
  describe 'POST /api/v2/order/delete' do
    let!(:order) { create(:order_bid, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '12.326'.to_d, volume: '3.14', origin_volume: '12.13', locked: '20.1082', origin_locked: '38.0882', member: member) }

    context 'succesful' do
      before do
        member.get_account(Peatio.base_fiat_ccy_sym.downcase).update_attributes(locked: order.price * order.volume)
      end
  ...
  describe 'POST /api/v2/orders/clear' do
    before do
      create(:order_ask, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '12.326', volume: '3.14', origin_volume: '12.13', member: member)
      create(:order_bid, currency: "btc#{Peatio.base_fiat_ccy.downcase}", price: '12.326', volume: '3.14', origin_volume: '12.13', member: member)

      member.get_account(:btc).update_attributes(locked: '5')
      member.get_account(Peatio.base_fiat_ccy_sym.downcase).update_attributes(locked: '50')
    end
