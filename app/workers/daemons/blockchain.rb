        ...
        while running
          begin
            ...
            (from_block..to_block).each do |block_id|
              ...
              block_json, deposits, withdrawals = bc_service.process_block(block_id)
              logger.warn message: "Fetch #{block_json.transactions.count} transactions in block number #{block_id}."
              binding.pry
              logger.info message: "Deposits created", models: deposits
              logger.info message: "Created withdrawals: #{withdrawals}"
